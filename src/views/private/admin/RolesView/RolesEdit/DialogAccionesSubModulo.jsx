...
import { startAddAccionSubModulo, startRemoveAccionSubModulo } from 'redux/actions/permisos';
...
const DialogAccionesSubModulo = props => {
	...
	useEffect(() => {
				...
				if (ele.modulo === selectMod.tag) {
					...
					const promises_ = ele.permisos.map(async ele_ => {
						...
						if (ele_.submodulo === selectSubMod.tag) {
							// Recorremos las acciones permitidas del submódulo
							const promises__ = ele_.acciones.map(ele__ => {
								return `${selectMod.tag}-${selectSubMod.tag}-${ele__}`;
							});
							const accionesPermitidas = await Promise.all(promises__);
							// Guardamos las acciones permitidas del submódulo
							setSelectedAccions(accionesPermitidas);
							return null;
						}
					});
		...
		// Si existe un socket o cambia, Si el contenido de permisos es mayor que cero y si existe un módulo seleccionado y un submódulo seleccionado
		if (socket && permisos.length > 0 && selectMod?.tag && selectSubMod?.tag) {
	...
	}, [socket, permisos, selectMod, selectSubMod]);
	...
	const handleSelectAccion = (evt, row) => {
		...
		const currentIndex = selectedAccions.indexOf(`${selectMod.tag}-${selectSubMod.tag}-${row.nombre}`);
		const newSelectedAccions = [...selectedAccions];

		// Evaluamos el estado del checkbox y almacenamos los arrays
		if (checked) {
			newSelectedAccions.push(`${selectMod.tag}-${selectSubMod.tag}-${row.nombre}`);
			dispatch(startAddAccionSubModulo(permisos, selectMod.tag, selectSubMod.tag, row.nombre));
		} else {
			newSelectedAccions.splice(currentIndex, 1);
			dispatch(startRemoveAccionSubModulo(permisos, selectMod.tag, selectSubMod.tag, row.nombre));
		}
	...
	return (
		<Dialog fullWidth={true} maxWidth="md" open={open} onClose={handleClose} aria-labelledby="accionessubmods-list">
			<DialogTitle id="accionessubmods-list">Acciones del SubMódulo {selectSubMod.nombre}</DialogTitle>
			<DialogContent>
				<div>
