...
const ModulosEditForm = props => {
  ...
  const { orden, tag, nombre, descripcion, type, icon, url, estado } =
    formValues
  ...
  // Estado de carga de los datos de los módulos
  ...
  useEffect(() => {
    ...
    const getModulo = async () => {
      ...
      const result = await fetchData(`admin/modulos/${id}`, {
        isTokenReq: true
      })
  ...
  // Si los datos del módulo están cargando
  if (loading) {
