...
/*******************************************************************************************************/
// Definimos la Vista del componente Admin - Módulos Header //
/*******************************************************************************************************/
const ModulosHeader = props => {
	...
	// Si hay un cambio en la lista de módulos
	useEffect(() => {
	...
	const handleInputSearch = async evt => {
		...
		const promises = list
			...
			.filter(ele => {
				return (
					normalizar(ele.orden.toString()).includes(value_) ||
					normalizar(ele.tag).includes(value_) ||
					normalizar(ele.nombre).includes(value_) ||
					normalizar(ele.type).includes(value_)
				);
			});
		...
		<div className="flex flex-1 w-full items-center justify-between">
			...
			{(rol.super || (accionesPerm && accionesPerm.indexOf('crear') !== -1)) && (
				<Animate animation="transition.slideRightIn" delay={300}>
					...
					>
						<span className="hidden sm:flex">Añadir Nuevo Módulo</span>
