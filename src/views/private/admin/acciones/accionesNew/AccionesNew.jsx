...
import AccionesNewHeader from './AccionesNewHeader';
import AccionesNewForm from './AccionesNewForm';
...
const AccionesNew = () => {
	...
	const handleSubmit = async () => {
		...
		const result = await fetchData('admin/acciones', { isTokenReq: true }, 'POST', formValues);
	...
	return (
		<Formsy onValidSubmit={handleSubmit} onValid={enableButton} onInvalid={disableButton}>
			<PageCarded
				...
				header={<AccionesNewHeader isFormValid={isFormValid} />}
				contentToolbar={
					<div className="px-16 sm:px-24">
						<h2>Registro</h2>
					</div>
				}
				content={<AccionesNewForm formValues={formValues} handleInputChange={handleInputChange} />}
				innerScroll
			/>
		</Formsy>
	);
