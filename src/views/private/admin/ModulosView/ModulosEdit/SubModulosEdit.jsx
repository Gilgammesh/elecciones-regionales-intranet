...
import { startAddSubmodulo, startUpdateSubmodulo } from 'redux/actions/submodulos';
...
const SubModulosEdit = () => {
	...
	const validateAddSubMod = () => {
		if (submodulos.filter(ele => parseInt(ele.orden, 10) === parseInt(orden, 10)).length > 0) {
			Swal.fire({
	...
	const validateUpdateSubMod = () => {
		...
		if (submodulos_.filter(ele => parseInt(ele.orden, 10) === parseInt(orden, 10)).length > 0) {
			Swal.fire({
				title: 'Ya existe un submódulo con este orden',
				icon: 'error'
			});
	...
	return (
		<div>
			<hr style={{ height: 1, borderWidth: 0, color: 'gray', backgroundColor: 'gray' }} />
