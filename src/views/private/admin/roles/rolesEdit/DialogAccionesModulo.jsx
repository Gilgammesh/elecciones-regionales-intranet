...
import { startAddAccionModulo, startRemoveAccionModulo } from 'redux/actions/permisos';
...
const DialogAccionesModulo = props => {
	...
	useEffect(() => {
		...
		const getAcciones = async () => {
			...
			const promises = permisos.map(async ele => {
				...
				if (ele.modulo === selectMod.tag) {
					// Recorremos las acciones permitidas del módulo
					const promises_ = ele.acciones.map(ele_ => {
						return `${selectMod.tag}-${ele_}`;
					});
					const accionesPermitidas = await Promise.all(promises_);
					// Guardamos las acciones permitidas del módulo
		...
		// Si existe un socket o cambia, Si el contenido de permisos es mayor que cero y si existe un módulo seleccionado
		if (socket && permisos.length > 0 && selectMod?.tag) {
	...
	const handleSelectAccion = (evt, row) => {
		...
		const currentIndex = selectedAccions.indexOf(`${selectMod.tag}-${row.nombre}`);
	...
	return (
		<Dialog fullWidth={true} maxWidth="lg" open={open} onClose={handleClose} aria-labelledby="accionesmod-list">
			<DialogTitle id="accionesmod-list">Acciones del Módulo {selectMod.nombre}</DialogTitle>
			<DialogContent>
				<div>
										...
										<TableRow className="h-32" hover tabIndex={-1} key={row._id}>
											...
											<TableCell className="py-2" component="th" scope="row" align="center">
												<Checkbox
													...
													checked={
														selectedAccions.indexOf(`${selectMod.tag}-${row.nombre}`) !== -1
													}
