...
import { FormControl, InputLabel, MenuItem, Select, Paper, Icon, Input } from '@material-ui/core';
import { fetchData } from 'services/fetch';
import { normalizar } from 'helpers/texts';
import { startSetCentrosVotacionLocal } from 'redux/actions/centrosVotacion';

/*******************************************************************************************************/
// Definimos la Vista del componente Centros de Votación ToolBar - Locales //
/*******************************************************************************************************/
const CentrosVotacionToolBarLocal = () => {
	...
	const { departamento, provincia, distrito, local } = useSelector(state => state.centros_votacion);
	...
	useEffect(() => {
		...
		const getLocales = async (dpto, prov, dist) => {
			...
			const result = await fetchData(
				`centros-votacion/locales?departamento=${dpto}&provincia=${prov}&distrito=${dist}`,
				{
					isTokenReq: true
				}
			);
	...
	const handleChange = evt => {
		...
		dispatch(startSetCentrosVotacionLocal(value));
	};
...
/*******************************************************************************************************/
export default CentrosVotacionToolBarLocal;
