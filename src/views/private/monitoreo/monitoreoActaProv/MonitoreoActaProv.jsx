...
import MonitoreoActaProvHeader from './MonitoreoActaProvHeader'
import MonitoreoActaProvToolBar from './MonitoreoActaProvToolBar'
import MonitoreoActaProvTable from './MonitoreoActaProvTable'
...
/*******************************************************************************************************/
// Definimos la Vista del componente Acta Provincial //
/*******************************************************************************************************/
const MonitoreoActaProv = () => {
  ...
  useEffect(() => {
    ...
    const getOrganizaciones = async () => {
      const result = await fetchData(
        `monitoreo/acta-provincial?departamento=${row.departamento._id}&provincia=${row.provincia._id}&distrito=${row.distrito._id}`,
        {
          isTokenReq: true
        }
      )
      if (mounted && result && result.data.status) {
        setOrganizaciones(
          result.data.list.map(org => {
            return { ...org, votos_alc_prov: 0, votos_alc_dist: 0 }
          })
        )
        setLoading(false)
      }
  ...
  return (
    <Formsy onValidSubmit={handleSubmit}>
      <PageCarded
        ...
        header={<MonitoreoActaProvHeader handleSubmit={handleSubmit} />}
        contentToolbar={<MonitoreoActaProvToolBar row={row} />}
        content={
          <MonitoreoActaProvTable
            row={row}
            loading={loading}
            nulos={nulos}
            setNulos={setNulos}
...
/*******************************************************************************************************/
export default MonitoreoActaProv
